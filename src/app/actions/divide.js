...
exports.divide = (firstValue, secondValue) => {
    ...
    if (exports.findBiggestValueString(secondValue, firstValue)) {
        lengthOutput--;
    }
    do {
        ...
        if (lengthOutput !== 0 && firstValue[0] !== '0' && exports.findBiggestValueString(secondValue, firstValue) && firstValue.length > secondValue.length) {
            if (!isFirst) {
                readableStream.push(count.toString());
                lengthOutput--;
            }
        ...
        while (exports.findBiggestValueString(firstValue, secondValue)) {
            firstValue = exports.subtract(firstValue, secondValue);
            count++;
        }
        if (firstValue[0] !== '0') {
            isFirst = true;
...
exports.subtract = (firstValueMain, secondValueMain) => {
    let subOne = false;
    let addTen = false;
    const isMinus = exports.findBiggestValueString(secondValueMain, firstValueMain);
    ...
    for (let i = 1; i <= secondValue.length; i++) {
        const secondNumber = Number(secondValue[secondValue.length - i]);
        if (addTen) {
            subOne = true;
            addTen = !addTen;
        }
...
const subFn = (value, index, subValue, flag) => value.substr(0, index)
    + (flag ? (Number(value[index]) + 10 - subValue) : (Number(value[index]) - subValue))
    + value.substr(index + 1, value.length - index - 1);
exports.findBiggestValueString = (firstValue, secondValue) => {
    if (firstValue.length === secondValue.length) {
...
const removeLeadingZero = (value) => value.substr(1, value.length - 1);
